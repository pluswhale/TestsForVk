...
public class MainTest extends SetupConnection {
    ...
    @Test(priority = 3)
    public void validateOneElementOfLeftBar() {

        String expected = shareListOfElementsFromExcel.get(0);
        String actual = shareListOfElementsfromWebLeftBar.get(0).getText().trim();
        Assert.assertEquals(expected,actual);
    }

    @Test(priority = 4)
    public void validateElementsOfLeftBar() {

        String expected;
        String actual;
        for(int i = 0; i <= shareListOfElementsfromWebLeftBar.size() - 1; i++) {
            expected = shareListOfElementsFromExcel.get(i);
            actual = shareListOfElementsfromWebLeftBar.get(i).getText().trim(); //
            Assert.assertEquals(expected,actual, "Not all elements matched");
        }
        logger.warn("It's work, but two element not visible at the main page - Stickers, and Orders. " + " ClassName : " + getClass());

    }

    @Test(priority = 10)
    public void checkTransitionOnLeftBar() throws InterruptedException {

        MainPage mainpage = new MainPage(driver);
        mainpage.clickOnEveryElementOnLeftBat();
        logger.fatal("It doesn't work because not all elements pass for test" + " ClassName : " + getClass());
    }


    @Test(priority = 5)
    public void tryPostAndCheck() throws InterruptedException {

        MainPage mainpage = new MainPage(driver);
        mainpage.clickOnTestField();
        logger.warn("It works" + " ClassName : " + getClass());
    }

    @Test(priority = 6)
    public void tryToWriteSomeone() throws InterruptedException {

        MainPage mainPage = new MainPage(driver);
        mainPage.writeToSomeFriend();
